...
public class GUIManual extends JFrame {
    ...
    private void accionDashboardRes() {
        ...
        xddd panel = new xddd();
        jPanelMain.removeAll();
        panel.actualizar(resultadosDAO.getPartidos(), resultadosDAO.calcularPromGoles(), resultadosDAO.masGoles(), resultadosDAO.menosGoles(), resultadosDAO.ganados(), resultadosDAO.empates(), "Belgica", "Arabia S", "Europa", "Oceania", "Croacia", "Arabia S");
        ...
        try {
            ...
            // Se iteran cada una de las líneas del archivo
             while (entrada.hasNext()) {
                ...
                if (resultadosDAO.registrarResultados(resultado)) {
    ...
    public void pintarTablaResultados() {
        ...
        buscar.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String equipo = field.getText();
                resultados = resultadosDAO.getResultadosMatrizBuscar(equipo);
                //JOptionPane.showMessageDialog(null, seleccionDAO.getCantEquipo());
                if (selecciones!= null){
                    pintarTablaResultados();
                }
